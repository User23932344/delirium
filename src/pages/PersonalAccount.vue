    ...
    <div v-if="activeTab === 'settings'">
      <div class="settings">
        ...
        <div class="box2">
          ...
          <input :type="showPassword1 ? 'text' : 'password'" class="pass" placeholder="Старый пароль" />
          <input :type="showPassword2 ? 'text' : 'password'" class="pass" placeholder="Новый пароль" />
          <input :type="showPassword3 ? 'text' : 'password'" class="pass" placeholder="Повторите новый пароль" />
          <button class="change">Изменить</button>
        </div>
        <div class="box3">
      ...
      <div v-if="userData">
        <div class="achievements">
          ...
          <div class="field">
            ...
            <p class="variable">{{ userData?.stats?.current_survival ?? '—' }}</p>
          </div>
          <div class="field">
            <p class="fieldname">Общее выживание</p>
            <p class="variable">{{ userData?.stats?.total_survival ?? '—' }}</p>
          </div>
          <div class="field">
            <p class="fieldname">Пройдено пешком</p>
            <p class="variable">{{ userData?.stats?.walked_distance ?? '—' }} м</p>
          </div>
          <div class="field">
            <p class="fieldname">Пройдено на транспорте</p>
            <p class="variable">{{ userData?.stats?.vehicle_distance ?? '—' }} м</p>
          </div>
          <div class="field">
            <p class="fieldname">Убито игроков</p>
            <p class="variable">{{ userData?.stats?.players_killed ?? '—' }}</p>
          </div>
          <div class="field">
            <p class="fieldname">Выстрелы в голову</p>
            <p class="variable">{{ userData?.stats?.headshots ?? '—' }}</p>
          </div>
          <div class="field">
            <p class="fieldname">Попаданий всего</p>
            <p class="variable">{{ userData?.stats?.total_hits ?? '—' }}</p>
          </div>

          <div class="nameblock2">Достижения</div>
          <div v-if="Array.isArray(userData.achievements)" v-for="(achievement, index) in userData.achievements"
            :key="index" class="field">
...
const fetchUserData = async (userId) => {
  try {
    ...
    userData.value = {
      ...userDataResponse,
      statistics: {
        current_survival: achievementsData.current_survival,
        total_survival: achievementsData.total_survival,
        walked_distance: achievementsData.walked_distance,
        vehicle_distance: achievementsData.vehicle_distance,
        players_killed: achievementsData.players_killed,
        headshots: achievementsData.headshots,
        total_hits: achievementsData.total_hits,
      },
      achievements: achievementsData.achievementsList, // предполагается, что это массив достижений
    };
